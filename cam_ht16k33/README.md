...
## HEX Counter mit 8x8 font
```cpp

#define ht16k33_addr  0x70
#define demo1_Pause   500
...
int laufrichtung = lr_DOWN;
int counterDemo = 5;
uint8_t counterDemoMax = 0xff;
...
void loop() {
  for (uint8_t x = 0; x < 8; x++) {
    uint8_t highNibble = (counterDemo >> 4) % 16;
    uint8_t lowNibble  = counterDemo % 16;
    uint8_t spalte[] = {
      x * 2 + 0,
      x * 2 + 1
    };
    ht16k33_1.setDisplayRaw(spalte[0], cam__8x8_font_C64_hex[highNibble][x]);
    ht16k33_1.setDisplayRaw(spalte[1], cam__8x8_font_C64_hex[lowNibble][x]);
  }
  ht16k33_1.sendLed();
  delay(demo1_Pause);
  
  counterDemo += laufrichtung;
  if (counterDemo >= counterDemoMax) {
    counterDemo = 0;
  }
  if (counterDemo < 0) {
    counterDemo = counterDemoMax;
  }
}
```
